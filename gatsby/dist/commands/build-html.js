...
const doBuildPages = async (rendererPath, pagePaths, activity, workerPool, stage) => {
  try {
    ...
    if (error !== null && error !== void 0 && (_error$context = error.context) !== null && _error$context !== void 0 && _error$context.path) {
      ...
      const errorMessage = `Truncated page data information for the failed page "${error.context.path}": ${JSON.stringify(modifiedPageDataForErrorMessage, null, 2)}`; // This is our only error during preview so customize it a bit + add the
      // pretty build error.
